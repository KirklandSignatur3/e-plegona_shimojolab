...
Shader "w0w/SH_Sparks_Vert_Color2" 
{
	Properties 
	{
	...
	{	
		...
		Pass
		{
			...
			{		
				...
				o.normal = v.normal;//normalize(cross(modifiedTangent, modifiedBinormal));
				o.vertex = UnityObjectToClipPos(v.vertex);//UnityObjectToClipPos(float4(pos.xyz, 0));
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				#ifdef UNITY_PROCEDURAL_INSTANCING_ENABLED
					color = particles[unity_InstanceID].color2;
				...
				float fresnel = saturate(i.vdotn);
				return (fresnel) * color * emission + color;
			}
