...
Shader "w0w/SH_Particles_Vert_Color3" 
{
	Properties 
	{
	...
	{	
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				#ifdef UNITY_PROCEDURAL_INSTANCING_ENABLED
					color = particles[unity_InstanceID].color3;
					unique = particles[unity_InstanceID].unique;
					if(particles[unity_InstanceID].life <= 0 && particles[unity_InstanceID].death <= 0) clip(-1.0);
				#endif

				float2 textureCoordinate = i.screenPosition.xy / i.screenPosition.w;
				float r = random(float2(0, unique)) * 0.1;
				textureCoordinate += float2(r, r);
    			fixed4 col = tex2D(_MainTex, textureCoordinate);
    			fixed4 col2 = tex2D(_MainTex2, textureCoordinate);
				fixed4 c = float4(0, 0, 0, 0);
				if(random(float2(unique, 0)) < 0.5) c = col;
				else c = col2;
				return saturate(color * float4(c.r, c.r, c.r, c.r));
			}
