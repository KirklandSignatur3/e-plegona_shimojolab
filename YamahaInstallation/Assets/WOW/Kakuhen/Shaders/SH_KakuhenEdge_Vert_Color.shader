Shader "w0w/SH_KakuhenEdge_Vert_Color" 
{
	Properties 
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float unique = 0;
					...
					a = SimplexNoise(i.vertex.xyz * 0.005 + rand_range(unity_InstanceID, unique, 1000) + _Time * 0.00001);
					if(particles[unity_InstanceID].death <= 0) clip(-1.0);
				#endif

				return lerp(color1, color2, a) * emission;
			}
