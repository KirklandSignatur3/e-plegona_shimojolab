Shader "w0w/SH_KakuhenLight_Vert_Color" 
{
	Properties 
	{
	...
	{	
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				#ifdef UNITY_PROCEDURAL_INSTANCING_ENABLED
					color1 = particles[unity_InstanceID].color;
					color2 = particles[unity_InstanceID].color2;
					unique = particles[unity_InstanceID].unique;
					a = SimplexNoise(i.vertex.xyz*0.005 + rand_range(unity_InstanceID, unique, 1000) + _Time * 0.00001);// + 0.5;
					if(particles[unity_InstanceID].death <= 0) clip(-1.0);
				#endif

				return saturate(lerp(color1, color2, a));
			}
