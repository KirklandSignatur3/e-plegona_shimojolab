Shader "w0w/SH_Instancing_Phong" 
{
	Properties 
	{
	...
	SubShader 
    {
		...
		Pass
		{
			...
			//#pragma surface surf Standard addshadow fullforwardshadows vertex:vert// tessphong:_Phong
			...
			{
				float noise = SimplexNoise(pos * 0.5 + rand_range(id, 0, 1000) + _Time *0.3);
			...
			{		
				...
				o.viewDir = normalize(_WorldSpaceCameraPos - mul(unity_ObjectToWorld, float4(pos.xyz, 0)));
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float specular = pow(saturate(dot(i.viewDir, R)), 1.0);

				float4 color = float4(1.0,1.0,1.0,1.0);
				#ifdef UNITY_PROCEDURAL_INSTANCING_ENABLED
					color = particles[unity_InstanceID].color;
					if(particles[unity_InstanceID].life <= 0) clip(-1.0);
				#endif
				
				return ((diffuse + specular) * _LightColor0 + float4(0.1,0.1,0.1,1.0));// * texColor;
			}
